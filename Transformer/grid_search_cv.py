...
def main():
    parser = argparse.ArgumentParser(
        description='Grid Search (manual backprop Transformer) with K-Fold CV'
    )
    ...
    # Custom grid search 参数 (仅在 --search_mode custom 时用)
    ...
    if args.search_mode == 'quick':
        ...
        quick_grid: Dict[str, List[Any]] = {
            ...
            'pos_encoding': ['sinusoidal', 'learnable', 'conditional'],
            'mlp_ratio': [None],
            'augmentation': [None, 'light'],
            'movement_features': [None, 'concat'],
        }
        param_grid = None  # 只是为了后面类型一致，不会用到
    else:  # custom / standard / extensive（这里先统一走 custom 的逻辑，你可以再细分）
        param_grid: Dict[str, List[Any]] = {}
    ...
    if args.search_mode == 'quick':
        # 分阶段的超参数组：
        # Stage 1: 结构相关（模型容量）
        # Stage 2: 数据表示相关（augmentation / movement_features）
